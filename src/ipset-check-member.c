...
main (int argc, char *argv[])
{
  ...
  /* Accept TYPE_LEN chars + NULL terminator. */
#define TYPE_LEN 10
  char type[TYPE_LEN + 1];
  memset(type, 0, TYPE_LEN);
  strncpy(type, argv[1], TYPE_LEN + 1);
  type[TYPE_LEN] = 0;

  /* Accept MEMBER_LEN chars + NULL terminator. */
#define MEMBER_LEN 40
  char member[MEMBER_LEN + 1];
  memset(member, 0, MEMBER_LEN);
  strncpy(member, argv[2], MEMBER_LEN + 1);
  member[MEMBER_LEN] = 0;
